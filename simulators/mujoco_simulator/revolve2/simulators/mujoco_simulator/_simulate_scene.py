...
from .viewers._viewer_type import ViewerType
...
def simulate_scene(
    ...
    if not headless or record_settings is not None:
        match viewer_type:
            case viewer_type.CUSTOM:
                from .viewers._custom_mujoco_viewer import CustomMujocoViewer

                viewer = CustomMujocoViewer
            case viewer_type.NATIVE:
                from .viewers._native_mujoco_viewer import NativeMujocoViewer
    ...
    while (time := data.time) < (
        ...
        if time >= last_control_time + control_step:
            ...
            should_stop = getattr(scene.handler, "should_stop", None)
            if should_stop is not None and should_stop(simulation_state):
                if sample_step is not None:
