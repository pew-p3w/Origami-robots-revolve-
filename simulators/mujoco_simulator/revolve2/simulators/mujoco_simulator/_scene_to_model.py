...
def scene_to_model(
    ...
    for mbs_i, (
        ...
        # MuJoCo 3.x removed 'actuatorfrcrange' from <joint>; strip it for compatibility.
        clean_urdf = re.sub(r'\s*actuatorfrcrange="[^"]*"', "", urdf)
        multi_body_system_model = mujoco.MjModel.from_xml_string(clean_urdf)
...
def _strip_actuatorfrcrange(filename):
    with open(filename, "r") as f:
        xml = f.read()
    import re as _re
    xml = _re.sub(' actuatorfrcrange="[^"]*"', "", xml)
...
def _create_tmp_file(multi_body_system_model: mujoco.MjModel) -> mjcf.RootElement:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            _strip_actuatorfrcrange(mjcf_file.name)
